"""Automower error-code → human text lookup.

Sourced from Husqvarna's Automower Connect API error table. Used to attach a
readable ``error_text`` field alongside the numeric ``error_code`` so the
dashboard can show "Cutting system blocked" instead of "24".
"""
...
def error_text(code: int | None) -> str:
    """Human-readable text for an error code (``"none"`` when code is 0/None)."""
    if not code:
        return "none"
